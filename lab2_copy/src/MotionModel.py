...
class KinematicMotionModel:
  ...
  def motion_cb(self, msg):
    ...
    for particle in particles:
        ...
        #calculate theta
        theta = initial_theta + (current_speed/car_length)*np.sin(2*beta)*TEST_DT

        #update particle positions (x,y,theta)
        particle[0] += (car_length/np.sin(2*beta)) * (np.sin(theta) - np.sin(initial_theta))     #+ random.gauss(0,0,KM_X_FIX_NOISE) update x
        particle[1] += (car_length/np.sin(2*beta)) * (-1*np.cos(theta) + np.cos(initial_theta))  #+ random.gauss(0,0,KM_Y_FIX_NOISE) update y
