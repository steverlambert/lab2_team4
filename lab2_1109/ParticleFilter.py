...
class ParticleFilter():
  ...
  def initialize_global(self):
    ...
    particles[:,2] =  - np.pi + 2*np.pi * np.random.rand(self.N_PARTICLES, 1)[:,0]
  ...
  def expected_pose(self):
    #get a local particles and weights array
    particles = np.zeros((self.N_PARTICLES,3))
    ...
    #Look for index of largest weight
    max_index = 0
  ...
  def clicked_pose_cb(self, msg):
    ...
    self.particles[:,0] = pose_x + gauss_var * np.random.randn(self.N_PARTICLES, 1)[:,0]
    self.particles[:,1] = pose_y + gauss_var * np.random.randn(self.N_PARTICLES, 1)[:,0]
    self.particles[:,2] = pose_theta + gauss_var_theta * np.random.randn(self.N_PARTICLES, 1)[:,0] 
