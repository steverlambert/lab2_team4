...
class KinematicMotionModel:
  ...
  def motion_cb(self, msg):
      ...
      p_count = self.particles.shape[0] # number of particles
      L = self.CAR_LENGTH
      W = CAR_WIDTH
  
      dt = (start - self.last_vesc_stamp).to_sec()
      ...
      # Initialize control vectors and add noise
      noisy_speed = np.zeros((p_count, 1))
      noisy_steer = np.zeros((p_count, 1))
      B = np.zeros((p_count, 1))
  
      noisy_speed[:] = cur_speed + KM_V_NOISE * np.random.randn(p_count, 1) 
      noisy_steer[:] = cur_steer + KM_DELTA_NOISE * np.random.randn(p_count, 1) 
     
  
   
      B[:,0] = np.arctan(.5 * np.tan(noisy_steer[:,0])) # angle from car's x-axis to steering angle
      
  
  
      # Initialize intermediate arrays
      thetas = np.zeros((p_count, 2))
      thetas[:,0] = self.particles[:,2]
  
      # Kinematics equations
      thetas[:,1] = thetas[:,0] + (noisy_speed[:,0]/L) * np.sin(2*B[:,0]) * dt
      self.particles[:,0] = self.particles[:,0] + L/np.sin(2*B[:,0]) * (np.sin(thetas[:,1]) - np.sin(thetas[:,0]))
      self.particles[:,1] = self.particles[:,1] + L/np.sin(2*B[:,0]) * (-np.cos(thetas[:,1]) + np.cos(thetas[:,0]))
  
      # Update particles array
      self.particles[:,2] = thetas[:,1]
      self.particles[:,0] = self.particles[:,0] + KM_X_FIX_NOISE * np.random.randn(p_count, 1)[:,0]
      self.particles[:,1] = self.particles[:,1] + KM_Y_FIX_NOISE * np.random.randn(p_count, 1)[:,0]
      self.particles[:,2] = self.particles[:,2] + KM_THETA_FIX_NOISE * np.random.randn(p_count, 1)[:,0] 
  
      
      # Loop through to limit theta to -pi to pi
