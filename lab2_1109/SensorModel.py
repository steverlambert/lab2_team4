...
class SensorModel:
  ...
  def lidar_cb(self, msg):
    ...
    # Copy ranges into new array and loop through for nans and 0.0s
    my_ranges = np.array(msg.ranges[0:len(msg.ranges):self.LASER_RAY_STEP], dtype = np.float32)
    if not isinstance(self.queries, np.ndarray): # Only copy the first time (not sure if this works)?
