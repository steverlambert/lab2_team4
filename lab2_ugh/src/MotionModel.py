...
class KinematicMotionModel:
  ...
  def motion_cb(self, msg):
    ...
    current_speed = (msg.state.speed - self.SPEED_TO_ERPM_OFFSET)/ self.SPEED_TO_ERPM_GAIN
    ...
    for particle in particles:
        #Make controls noisy
        current_speed += random.gauss(0.0,KM_V_NOISE)
        current_steering_angle += random.gauss(0.0,KM_DELTA_NOISE)

        #save initial theta
        initial_theta = particle[2]

        #calculate beta
        beta = np.arctan( (1.0/2.0)*np.tan(current_steering_angle) )

        #calculate theta
        theta = initial_theta + (current_speed/self.CAR_LENGTH)*np.sin(2*beta)*TEST_DT

        #update particle positions (x,y,theta)
        particle[0] += (self.CAR_LENGTH/np.sin(2*beta)) * (np.sin(theta) - np.sin(initial_theta))     #+ random.gauss(0,0,KM_X_FIX_NOISE) update x
        particle[1] += (self.CAR_LENGTH/np.sin(2*beta)) * (-1*np.cos(theta) + np.cos(initial_theta))  #+ random.gauss(0,0,KM_Y_FIX_NOISE) update y
        particle[2] = theta # + random.gauss(0,0,KM_THETA_FIX_NOISE) #update theta

        #Add position noise
        particle[0] += random.gauss(0.0, KM_X_FIX_NOISE)
        particle[1] += random.gauss(0.0, KM_Y_FIX_NOISE)
        particle[2] += random.gauss(0.0, KM_THETA_FIX_NOISE)

        #Limit theta between -pi and pi
        while particle[2] > np.pi:
            particle[2] = particle[2] + 2*np.pi
        while particle[2] < -1*np.pi:
            particle[2] = particle[2] - 2*np.pi
